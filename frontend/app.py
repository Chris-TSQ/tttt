...
def debug_data():
    ...
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT id, title, genres, region, rating FROM douban_top100_movies LIMIT 10;")
    rows = cursor.fetchall()
    cursor.close()
    conn.close()
    
    html = "<html><body><h2>Debug: First 10 Movies</h2><table border='1'>"
    html += "<tr><th>ID</th><th>Title</th><th>Genres</th><th>Region</th><th>Rating</th></tr>"
    for r in rows:
        html += f"<tr><td>{r[0]}</td><td>{r[1]}</td><td>{repr(r[2])}</td><td>{repr(r[3])}</td><td>{r[4]}</td></tr>"
...
def plot1():
    try:
        ...
        print(f"Plot 1: {len(avg)} genres found")
        print(f"Plot 1: Top genres: {list(avg.head(5).index)}")
        print(f"Plot 1: Rating range: {avg.min():.2f} - {avg.max():.2f}")
...
def plot2():
    try:
        ...
        print(f"Plot 2: Loaded {len(df)} rows")
        ...
        # Split genres
        df_ex = df.assign(genres_list=df["genres_en"].apply(split_genres)).explode("genres_list")
        df_ex = df_ex.drop(columns=["genres_en", "genres"])  # Drop original columns
        df_ex = df_ex.rename(columns={"genres_list": "genres_en"})
        df_ex = df_ex[df_ex["genres_en"] != ""].reset_index(drop=True)
        
        print(f"Plot 2: After explode: {len(df_ex)} rows")
        ...
        print(f"Plot 2: {len(counts)} genres found")
        print(f"Plot 2: Top 5 counts: {dict(counts.head(5))}")
...
def plot3():
    try:
        ...
        print(f"Plot 3: Loaded {len(df)} rows")
        ...
        print(f"Plot 3: After explode: {len(df_ex)} rows")
        print(f"Plot 3: Unique genres: {df_ex['genres_en'].nunique()}")
...
def plot4():
    try:
        ...
        print(f"Plot 4: Loaded {len(df)} rows")
        
        # Translate genres and regions
        df["genres_en"] = df["genres"].apply(lambda x: translate_text(x, GENRE_MAP))
        df["region_en"] = df["region"].apply(lambda x: translate_text(x, REGION_MAP))
        
        # Split genres
        df_ex = df.assign(genres_list=df["genres_en"].apply(split_genres)).explode("genres_list")
        df_ex = df_ex.drop(columns=["genres_en", "genres", "region"])  # Drop original columns
        df_ex = df_ex.rename(columns={"genres_list": "genres_en"})
        df_ex = df_ex[df_ex["genres_en"] != ""].reset_index(drop=True)
        
        print(f"Plot 4: After explode: {len(df_ex)} rows")
        
        # Create pivot table
        pivot = df_ex.pivot_table(index='genres_en', columns='region_en', values='rating', aggfunc='mean')
        
        print(f"Plot 4: Pivot shape = {pivot.shape}")
